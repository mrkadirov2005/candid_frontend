...
type AcceptInviteAdminForm = {
    name: string;
    universityId: string;
    password: string;
};

export function AcceptInviteUniversityAdminPage({ userData }: { userData: JWT_RESPONSE & { userId?: string } }) {
    const navigate = useNavigate();
    const { register, handleSubmit, formState: { errors } } = useForm<AcceptInviteAdminForm>();

    const acceptInviteMutation = useMutation({
        mutationFn: async (data: AcceptInviteAdminForm) => {
            const baseUrl = import.meta.env.VITE_API_BASE_URL || 'http://localhost:8050';
            const response = await fetch(`${baseUrl}${endpoints.university_admin.accept_invite}`, {
                method: 'POST',
    ...
    const onSubmit = (data: AcceptInviteAdminForm) => {
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} style={{ display: "flex", flexDirection: "column", gap: "1rem", maxWidth: "300px" }}>
            ...
            <div>
                <input
                    type="text"
                    placeholder="University ID (Optional UUID)"
                    {...register("universityId")}
                    style={{ padding: "0.5rem", width: "100%" }}
                />
                {errors.universityId && <span style={{ color: "red", fontSize: "0.8rem" }}>{errors.universityId.message}</span>}
            </div>
